...
contract LiquidityCertificate is ILiquidityCertificate, ERC721 {
  ...
  uint256 constant UNIT = 1e18; //multiplication base
  ...
  function split(uint certificateId, uint percentageSplit) external override returns (uint) {
    require(percentageSplit < UNIT, "split must be less than 100%");
    ...
    uint newCertLiquidity = (certData.liquidity * percentageSplit) / UNIT;
